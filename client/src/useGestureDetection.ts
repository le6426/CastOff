...
function isFistClosed(landmarks: NormalizedLandmark[]): boolean {
  ...
  // Thumb position is intentionally not checked — testing showed no
  // reliable, consistent threshold for "tucked" vs "extended" across
  // different cameras/setups, so curl-only detection is used instead.
